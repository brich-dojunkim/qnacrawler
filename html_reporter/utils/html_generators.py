# html_reporter/utils/html_generators.py (완전 업데이트 - 문의 모달 연결)
"""HTML 문자열 생성 함수들 - 완료율 칼럼 포함 + 문의 모달 연결"""
...
class HTMLGenerator:
    ...
    def generate_sub_categories_html(sub_categories: Dict, results: Dict = None, max_items: int = 10) -> str:
        """세부 카테고리 테이블 형태 HTML 생성 - 문의율 칼럼 포함 + 문의 모달 연결"""
        ...
        for category_name, count in sorted_categories[:max_items]:
            ...
            # 문의 모달 연결 - openInquiryModal 함수 사용
            html += f'''
                    <div class="sub-category-row" 
                        ...
                        <div class="sub-cat-cell">
                            <button class="sub-cat-action-btn" onclick="openInquiryModal('subcategory', '{safe_category_name}')" title="상세 문의 보기">
